...
export async function readProxies() {
  try {
    ...
    if (err.code === "ENOENT") {
      log.warn(
        "No proxies found in proxy.txt. Requests will proceed without a proxy."
      );
      return [];
    }
    throw err;
    ...
    try {
      ...
      if (currentProxy) {
        ...
        if (currentProxy.startsWith("socks5://")) {
          ...
          currentProxy.startsWith("http://") ||
          currentProxy.startsWith("https://")
        ) {
        ...
        log.warn(
          `No proxy available for wallet: ${address}. Proceeding without a proxy.`
        );
      }
      ...
      if (attempt < maxRetries) {
        ...
        log.error(
          `Failed to claim faucet for ${address} after ${maxRetries} attempts.`
        );
      }
    }
  }
